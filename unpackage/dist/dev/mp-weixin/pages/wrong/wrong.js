  ...
  setup(__props) {
    ...
    const loadWrongList = async () => {
      ...
      try {
        ...
        const res = await api_record.getWrongList({ user_id: userStore.userInfo.id, page: 1, page_size: 100 });
        common_vendor.index.hideLoading();
        if (res.code === 200) {
          wrongList.value = ((_b = res.data) == null ? void 0 : _b.list) || [];
          updateStatistics();
        }
      } catch (error) {
        common_vendor.index.hideLoading();
        common_vendor.index.__f__("error", "at pages/wrong/wrong.vue:108", "加载错题失败:", error);
        loadLocalWrongList();
      }
    };
    ...
    const goToQuestion = (item) => {
      common_vendor.index.navigateTo({
        url: `/pages/question/question?id=${item.questionId}&from=wrong`
      });
    };
          ...
          if (res.confirm) {
            if (userStore.isLoggedIn) {
              try {
                if (!((_a = userStore.userInfo) == null ? void 0 : _a.id))
                  return;
                await api_record.deleteWrong({ user_id: userStore.userInfo.id, question_id: id });
              } catch (error) {
                common_vendor.index.__f__("error", "at pages/wrong/wrong.vue:206", "删除失败:", error);
              }
            } else {
